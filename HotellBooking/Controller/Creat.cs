...
namespace HotellBooking.Controller
{
    public class Creat : ICrud
    {
        ...
        public void Run()
        {
            ...
            List<HotellRoom> availableCars = new List<HotellRoom>();

            foreach (var car in dbContext.HotellRooms.ToList())
            {
                bool carIsFree = true;
                foreach (var booking in dbContext.Bookings.Include(b => b.room).Where(b => b.room == car))
                {
            ...
            Console.WriteLine($" {bookingToCreate.DateTimeStart.ToShortDateString()}\t{bookingToCreate.DateTimeEnd.ToShortDateString()}\t{numberOfDays}");
            ...
            else
            {
                ...
                foreach (var car in availableCars.OrderBy(r => r.Id))
                {
                    Console.WriteLine($" {car.Id}\t\t{car.Type}\t\t{car.beds}");
                    Console.WriteLine(" ------------------------------------------------------------------");
                }
            }
            ...
            var carLicensPlateForBooking =Convert.ToInt32( Console.ReadLine());

            bookingToCreate.room = dbContext.HotellRooms
                .Where( c => c.Id == carLicensPlateForBooking)
                .FirstOrDefault();
...
//                foreach (var rum in dbContext.Bookings.ToList())
//                {
//                    bool RoomsFree = true;
//                    foreach (var booking in dbContext.Bookings.Include(b => b.room)
//                                 .Where(b =>
//                                 {
//                                     if (rum != null) return b.room == rum;
//                                 }))
//                    {
//                        for (var dt = booking.DateTimeStart; dt <= booking.DateTimeEnd; dt = dt.AddDays(1))
//                        {
...
//                bookingToCreate.room = dbContext.HotellRooms
//                    .Where(c => c.Type == roomsforbooking)
//                    .FirstOrDefault();
